...
class YahooFinance
  ...
  def self.read_quotes(symb_str, cols, retry_on_error = true)
    result = ''
    columns = "#{cols.map {|col| COLUMNS[col] }.join('')}"

    begin
      open("http://download.finance.yahoo.com/d/quotes.csv?s=#{URI.escape(symb_str)}&f=#{columns}") do |connection|
        result = CSV.parse(connection.read, :headers => cols)
      end
      return result
    rescue OpenURI::HTTPError
      self.read_quotes(symb_str, cols, false) if retry_on_error
    end
  end
  ...
  def self.read_historical(symbol, start_date, end_date, options, retry_on_error = true)
    ...
    begin
      ...
      if retry_on_error
        read_historical(symbol, start_date, end_date, options, false)
      end
      []
    end
  end
  ...
  def self.read_symbols(query)
    ...
    result = conn.read
    result.sub!('YAHOO.Finance.SymbolSuggest.ssCallback(', '').chomp!.chomp!(')\;')
